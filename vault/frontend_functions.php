 ...
 * @see LICENSE.txt
 *
 * This file: Front-end functions file (last modified: 2022.05.16).
 */
...
$CIDRAM['FetchComponentsLists'] = function (array &$Arr) use (&$CIDRAM): void {
    $Data = $CIDRAM['Updater-IO']->readFile($CIDRAM['Vault'] . 'installed.yml');
    if (strlen($Data)) {
...
$CIDRAM['IsActivable'] = function (array &$Component) use (&$CIDRAM): bool {
    return (
        ...
        $CIDRAM['Has']($Component['Used with'], ['imports', 'events', 'ipv4', 'ipv6', 'modules'])
    );
};
...
$CIDRAM['FilterLang'] = function (string $ChoiceKey) use (&$CIDRAM): bool {
    $Core = $CIDRAM['Vault'] . 'l10n' . DIRECTORY_SEPARATOR . 'core' . DIRECTORY_SEPARATOR . $ChoiceKey;
    $FrontEnd = $CIDRAM['Vault'] . 'l10n' . DIRECTORY_SEPARATOR . 'frontend' . DIRECTORY_SEPARATOR . $ChoiceKey;
...
$CIDRAM['FilterTheme'] = function (string $ChoiceKey) use (&$CIDRAM): bool {
    ...
    return (
        file_exists($CIDRAM['Vault'] . 'assets/frontend/' . $ChoiceKey . '/frontend.css') ||
        file_exists($CIDRAM['Vault'] . 'assets/core/template_' . $ChoiceKey . '.html')
    );
};
...
$CIDRAM['GetAssetPath'] = function (string $Asset, bool $CanFail = false) use (&$CIDRAM): string {
    ...
    if (file_exists($CIDRAM['Vault'] . 'assets/frontend/' . $Asset)) {
        return $CIDRAM['Vault'] . 'assets/frontend/' . $Asset;
    }
    if ($CanFail) {
        return '';
    }
...
$CIDRAM['UpdatesHandler-Update'] = function ($ID) use (&$CIDRAM): void {
    ...
    foreach (array_unique($ID) as $ThisTarget) {
        ...
        if (isset($CIDRAM['Components']['Meta'][$ThisTarget])) {
        ...
        if (
            ...
            for ($Iterate = 0; $Iterate < $Count; $Iterate++) {
                ...
                if (
                    ...
                    !($ThisFile = $CIDRAM['Request']($CIDRAM['Components']['RemoteMeta'][$ThisTarget]['Files']['From'][$Iterate]))
                ) {
                    $Iterate = 0;
                    $Rollback = true;
                    continue;
            ...
            if ($Rollback) {
                ...
                $CIDRAM['Updater-IO']->writeFile(
                    $CIDRAM['Vault'] . 'installed.yml',
                    $CIDRAM['YAML']->reconstruct($CIDRAM['Components']['Meta'])
                );
...
$CIDRAM['UpdatesHandler-Uninstall'] = function ($ID) use (&$CIDRAM): void {
    ...
    if (
        ...
        unset($CIDRAM['Components']['Meta'][$ID]);
...
$CIDRAM['UpdatesHandler-Repair'] = function ($ID) use (&$CIDRAM): void {
    ...
    foreach ($ID as $ThisTarget) {
        if (!isset($CIDRAM['Components']['Meta'][$ThisTarget]['Files']['To'])) {
            continue;
        }
        $BytesAdded = 0;
        $BytesRemoved = 0;
        ...
        if (!$RepairFailed && is_writable($CIDRAM['Vault'] . 'installed.yml')) {
