 ...
 * @see LICENSE.txt
 *
 * This file: Output generator (last modified: 2026.02.23).
 */
...
if ($CIDRAM['Config']['general']['force_hostname_lookup'] && !$CIDRAM['Config']['general']['maintenance_mode']) {
    $CIDRAM['Stage'] = '';
...
if ($CIDRAM['BlockInfo']['SignatureCount'] > 0) {
    ...
    if (!empty($CIDRAM['Banned']) && $CIDRAM['Config']['general']['ban_override'] > 400 && $CIDRAM['ThisStatusHTTP'] = $CIDRAM['GetStatusHTTP']($CIDRAM['Config']['general']['ban_override'])) {
        ...
        /** Fetch appropriate status code. */
        if (!empty($CIDRAM['RL_Status']) && ($CIDRAM['BlockInfo']['SignatureCount'] * 1) === 1) {
            ...
            if ($CIDRAM['Config']['signatures']['suppress_rl']) {
