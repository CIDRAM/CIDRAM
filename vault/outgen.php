...
if ($CIDRAM['BlockInfo']['SignatureCount'] > 0) {
    ...
    if ($CIDRAM['Config']['recaptcha']['usemode'] || $CIDRAM['Config']['general']['empty_fields'] === 'include') {
        ...
        $CIDRAM['AddField']($CIDRAM['L10N']->getString('field_reCAPTCHA_state'), $CIDRAM['BlockInfo']['reCAPTCHA']);
    }
    ...
    if (!empty($CIDRAM['Config']['More Info']) && !empty($CIDRAM['BlockInfo']['ReasonMessage'])) {
        $CIDRAM['BlockInfo']['ReasonMessage'] .= '<br /><br />' . $CIDRAM['L10N']->getString('MoreInfo');
    ...
    if (!empty($CIDRAM['Banned']) && $CIDRAM['Config']['general']['ban_override'] !== 200 && (
        ...
        if (!$CIDRAM['template_file'] || !file_exists($CIDRAM['Vault'] . $CIDRAM['template_file'])) {
            ...
            $CIDRAM['HTML'] = '[CIDRAM] ' . $CIDRAM['L10N']->getString('denied');
        } else {
            ...
            if ($CIDRAM['Config']['general']['emailaddr']) {
                if ($CIDRAM['Config']['general']['emailaddr_display_style'] === 'default') {
                    $CIDRAM['BlockInfo']['EmailAddr'] =
                        ...
                        )) . '"><strong>' . $CIDRAM['L10N']->getString('click_here') . '</strong></a>';
                    $CIDRAM['BlockInfo']['EmailAddr'] = "\n  <p class=\"detected\">" . $CIDRAM['ParseVars']([
                        'ClickHereLink' => $CIDRAM['BlockInfo']['EmailAddr']
                    ], $CIDRAM['L10N']->getString('Support_Email')) . '</p>';
                    ...
                    ], $CIDRAM['L10N']->getString('Support_Email_2')) . '</p>';
                }
            }
            ...
            ) ? '' : '<br /><a href="' . $CIDRAM['Config']['legal']['privacy_policy'] . '">' . $CIDRAM['L10N']->getString('PrivacyPolicy') . '</a>';
    ...
    if (empty($CIDRAM['Flag Don\'t Log']) && (
        ...
        if ($CIDRAM['Config']['general']['logfile_apache']) {
            $CIDRAM['logfile_apacheData'] = ['d' => sprintf(
                "%s - - [%s] \"%s %s %s\" %s %s \"%s\" \"%s\"\n",
                $CIDRAM['BlockInfo']['IPAddr'],
                ...
                !file_exists($CIDRAM['Vault'] . $CIDRAM['LogFileNames']['logfile_apache']) || (
                    $CIDRAM['Config']['general']['truncate'] > 0 &&
            ...
            if ($CIDRAM['BuildLogPath']($CIDRAM['LogFileNames']['logfile_apache'])) {
                $CIDRAM['logfile_apacheData']['f'] = fopen(
                    $CIDRAM['Vault'] . $CIDRAM['LogFileNames']['logfile_apache'],
                    $CIDRAM['logfile_apacheData']['Mode']
                );
                fwrite($CIDRAM['logfile_apacheData']['f'], $CIDRAM['logfile_apacheData']['d']);
                fclose($CIDRAM['logfile_apacheData']['f']);
                if ($CIDRAM['logfile_apacheData']['Mode'] === 'w') {
                    $CIDRAM['LogRotation']($CIDRAM['Config']['general']['logfile_apache']);
                }
            }
            unset($CIDRAM['logfile_apacheData']);
        }

        /** Writing to the serialised logfile. */
        if ($CIDRAM['Config']['general']['logfile_serialized']) {
            ...
            $CIDRAM['WriteMode'] = (
                !file_exists($CIDRAM['Vault'] . $CIDRAM['LogFileNames']['logfile_serialized']) || (
                    $CIDRAM['Config']['general']['truncate'] > 0 &&
            ...
            if ($CIDRAM['BuildLogPath']($CIDRAM['LogFileNames']['logfile_serialized'])) {
                $CIDRAM['logfileSerialData'] = fopen(
                    $CIDRAM['Vault'] . $CIDRAM['LogFileNames']['logfile_serialized'],
                    $CIDRAM['WriteMode']
                );
