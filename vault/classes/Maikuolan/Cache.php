<?php
/**
 * A simple, unified cache handler (last modified: 2023.03.24).
 *
...
class Cache
{
     ...
     * @return bool True on success; False on failure.
     */
    public function incEntry($Key, $Value = 1, $TTL = 0)
    {
        if ($this->Using !== 'PDO') {
            $Key = $this->Prefix . $Key;
            $this->enforceKeyLimit($Key);
     ...
     * @return bool True on success; False on failure.
     */
    public function decEntry($Key, $Value = 1, $TTL = 0)
    {
        if ($this->Using !== 'PDO') {
            $Key = $this->Prefix . $Key;
            $this->enforceKeyLimit($Key);
     ...
     */
    public function clearExpired(array &$Data)
    {
        $Cleared = false;
        $Updated = [];
        $Now = time();
     ...
     * @return void
     */
    private function enforceKeyLimit(&$Key)
    {
        /**
