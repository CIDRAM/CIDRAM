...
if (!isset($this->CIDRAM['QueryVars']['cidram-page']) || $this->CIDRAM['QueryVars']['cidram-page'] !== 'logs' || empty($this->FE['PermissionsMap']['Logs'])) {
    die;
}
...
if (empty($this->CIDRAM['QueryVars']['logfile'])) {
    ...
    if ($this->FE['Paginate']) {
        ...
        while ($this->stepThroughBlocks($this->FE['logfileData'][0], $Needle, 0, $SearchQueryDecoded, '<', $WildCardHead, $WildCardFoot)) {
            if ($SearchQueryDecoded !== '') {
                $this->stepThroughBlocks($this->FE['logfileData'][0], $Needle, 0, '', '<', $SearchQueryDecoded, $WildCardHead, $WildCardFoot);
            }
            $Iterations++;
            if (!empty($this->CIDRAM['QueryVars']['search'])) {
        ...
        if (!$this->FE['Previous']) {
            $this->FE['Previous'] = $this->isolateFirstFieldEntry($this->FE['logfileData'][0], $this->FE['FieldSeparator']);
        }
        if ($this->FE['From'] === '') {
            $this->FE['From'] = $this->isolateFirstFieldEntry($this->FE['logfileData'][1], $this->FE['FieldSeparator']);
        }
    ...
    if (!empty($this->FE['logfileData']) && $SearchQueryDecoded !== '') {
        ...
        if ($this->FE['Paginate']) {
            ...
            $BlockEnd = 0;
            if ($this->FE['From'] === '') {
                $this->FE['From'] = $this->isolateFirstFieldEntry($this->FE['logfileData'], $this->FE['FieldSeparator']);
            }
            while (true) {
...
$this->FE['TextModeSwitchLink'] = \sprintf(
    '<div class="h2 s va"><label for="textMode">%1$s</label><br />' .
    ...
    '</div><div class="h2 s va"><div class="flexrow" style="gap:32px">' .
    ...
    '</div></div><div class="h2f s va" role="search">' .
    ...
    '</div></div><div class="h2f s va"></div>',
