 ...
 *
 * This file: Bot Or Browser User Agent Module (last modified: 2026.06.12).
 *
...
$this->CIDRAM['ModuleResCache'][$Module] = function () {
    ...
    if (isset($WhatToBlock['SuspectedBot'])) {
        ...
        if (
            ...
            $this->trigger(\preg_match('%U(?:2Bot|ASlinkChecker|CSD-Crawler\/|RL(?: Control\/|AppendBot|Checker|SS|(_)?Spider(_)?Pro\/|itor\.|y[ .]Warning\/)|TSE|XCrawlerBot|dmSearch|ltraseek|n(?:i(?:corn|sterBot\/|tek UniEngine)|windFetchor\/)|p(?:downerbot|flow|timeDog)|rl(?:Dispatcher|Trends|(file)?bot|stat))%', $this->BlockInfo['UA']), $Bot[0], $Bot[1]) ||
            ...
            $this->trigger(\preg_match('%Z(?:-Add Link Checker|IPCodeBuyBot|atka|e(?:albot|erchBot|ller|manta|us(?: Link Scout|\/|_))|ip(?:Commander|ppBot)|mEu|najdzFoto|o(?:o(?:kabot|m(?:Bot|Info bot)))|u(?:hause\\(ARexx\\)|mBot)|yBorg\/)%', $this->BlockInfo['UA']), $Bot[0], $Bot[1]) ||
            ...
            $this->trigger(\preg_match('%d(?:ata(?:gnionbot|minr\.com|provider)|e(?:adlinkchecker|voll)|i(?:enstspider|g(?:g\.com|italpebble)|s(?:avowfiles\.|coverybot\/|squs\.))|j(?:-research|bot)|l(?:cbot|vr\.)|o(?:coloc|mainsbot|tSemantic|wn(?:foreveryoneorjustme|notifier))|r(?:agonfly|rkpi\.|upact)|u(?:baiindex|mbot\/))%', $this->BlockInfo['UA']), $Bot[0], $Bot[1]) ||
            ...
            $this->trigger(\preg_match('%o(?:Bot\/\d|e(?:bot|gp)|iqBot|k(?:http|u-taka-lab-bot)|nline(?: link validator|-webceo-bot)|odlebot\.|pendi\.|utbrain|w(?:lin\.|nCloud))%', $this->BlockInfo['UA']), $Bot[0], $Bot[1]) ||
            ...
            $this->trigger(\preg_match('%z(?:eef\.|itebot|ootycoon\.)%', $this->BlockInfo['UA']), $Bot[0], $Bot[1]) ||
            ...
            if ($Opera) {
                ...
                if ($TokenOpera < $EOLOpera) {
                    if ($this->Configuration['bobuam']['token_difference_limit'] > 0 && $TokenOpera < ($EOLOpera - $this->Configuration['bobuam']['token_difference_limit'])) {
                        unset($Options['Opera:MarkForUseWithHCaptcha'], $Options['Opera:MarkForUseWithFriendlyCaptcha'], $Options['Opera:MarkForUseWithCloudflareTurnstile']);
                    }
                    $Caught[] = 'O';
            ...
            if ($Chromium) {
                ...
                if ($TokenChrome < $EOLChrome) {
                    if ($this->Configuration['bobuam']['token_difference_limit'] > 0 && $TokenChrome < ($EOLChrome - $this->Configuration['bobuam']['token_difference_limit'])) {
                        unset($Options['Chrome:MarkForUseWithHCaptcha'], $Options['Chrome:MarkForUseWithFriendlyCaptcha'], $Options['Chrome:MarkForUseWithCloudflareTurnstile']);
                    }
                    $Caught[] = 'C';
            ...
            if ($Edge) {
                ...
                if ($TokenEdge < $EOLEdge) {
                    if ($this->Configuration['bobuam']['token_difference_limit'] > 0 && $TokenEdge < ($EOLEdge - $this->Configuration['bobuam']['token_difference_limit'])) {
                        unset($Options['Edge:MarkForUseWithHCaptcha'], $Options['Edge:MarkForUseWithFriendlyCaptcha'], $Options['Edge:MarkForUseWithCloudflareTurnstile']);
                    }
                    $Caught[] = 'E';
            ...
            if ($Firefox) {
                ...
                if ($TokenFirefox < $EOLFirefox && $TokenFirefox !== $EOLFirefoxESR) {
                    if ($this->Configuration['bobuam']['token_difference_limit'] > 0 && $TokenFirefox < ($EOLFirefox - $this->Configuration['bobuam']['token_difference_limit'])) {
                        unset($Options['Firefox:MarkForUseWithHCaptcha'], $Options['Firefox:MarkForUseWithFriendlyCaptcha'], $Options['Firefox:MarkForUseWithCloudflareTurnstile']);
                    }
                    $Caught[] = 'F';
            ...
            if ($Safari) {
                ...
                if ($TokenSafari < $EOLSafari) {
                    if ($this->Configuration['bobuam']['token_difference_limit'] > 0 && $TokenSafari < ($EOLSafari - $this->Configuration['bobuam']['token_difference_limit'])) {
                        unset($Options['Safari:MarkForUseWithHCaptcha'], $Options['Safari:MarkForUseWithFriendlyCaptcha'], $Options['Safari:MarkForUseWithCloudflareTurnstile']);
                    }
                    $Caught[] = 'S';
    ...
    if (isset($WhatToBlock['Masquerade']) && $this->BlockInfo['SEC_CH_UA'] !== '') {
        ...
        if (isset($this->Tokens['Opera']) || isset($this->Tokens['OperaMobile'])) {
            if ($this->BlockInfo['SEC_CH_UA_MOBILE'] === '?1') {
                ...
                $Try = (int)($this->Tokens['Opera'] ?? $this->Tokens['OperaMobile']);
            }
            if (
