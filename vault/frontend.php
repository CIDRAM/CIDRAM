 ...
 * @see LICENSE.txt
 *
 * This file: Front-end handler (last modified: 2016.10.06).
 */
...
$CIDRAM['FE'] = array(
    ...
    'FrontEndData' =>
        (file_exists($CIDRAM['Vault'] . 'fe_assets/frontend.dat')) ?
        $CIDRAM['ReadFile']($CIDRAM['Vault'] . 'fe_assets/frontend.dat') :
        '',
    'UserList' => '',
    'SessionList' => '',
    'UserState' => 0,
...
if ($CIDRAM['FE']['UserListPos'] !== false) {
    $CIDRAM['FE']['UserList'] = substr(
        ...
        $CIDRAM['FE']['UserListPos'] + 12,
        $CIDRAM['FE']['SessionListPos'] - $CIDRAM['FE']['UserListPos'] - 14
    );
}
if ($CIDRAM['FE']['SessionListPos'] !== false) {
    $CIDRAM['FE']['SessionList'] = substr(
        $CIDRAM['FE']['FrontEndData'],
        $CIDRAM['FE']['SessionListPos'] + 18
    );
}

/** Determine whether the user has logged in. */
if (!empty($_COOKIE['CIDRAM-ADMIN'])) {
...
/** User hasn't logged in. Show them the login page. */
...
if ($CIDRAM['QueryVars']['cidram-page'] === 'docs') {
