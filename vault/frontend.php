...
if ($CIDRAM['FE']['UserState'] === 1) {
    ...
    if (empty($CIDRAM['Remote-YAML-PHP'])) {
        ...
        /** PHP latest stable. */
        $CIDRAM['FE']['info_php_stable'] = $CIDRAM['Remote-YAML-PHP-Array']['Stable'] ?? $CIDRAM['L10N']->getString('response_error');

        /** PHP latest unstable. */
        $CIDRAM['FE']['info_php_unstable'] = $CIDRAM['Remote-YAML-PHP-Array']['Unstable'] ?? $CIDRAM['L10N']->getString('response_error');

        /** PHP branch latest stable. */
        $CIDRAM['ThisBranch'] = 'php' . \PHP_MAJOR_VERSION . \PHP_MINOR_VERSION;
        $CIDRAM['FE']['info_php_branch'] = $CIDRAM['Remote-YAML-PHP-Array']['Branch'][$CIDRAM['ThisBranch']]['Latest'] ?? $CIDRAM['L10N']->getString('response_error');
    }
...
if ($CIDRAM['FE']['UserState'] !== 1 && $CIDRAM['FE']['CronMode'] === '') {
    ...
    foreach ($CIDRAM['Components']['Meta'] as $CIDRAM['Components']['Key'] => &$CIDRAM['Components']['ThisComponent']) {
        ...
        if ($CIDRAM['Components']['ThisComponent']['StatClass'] === '' && isset($CIDRAM['Components']['ThisComponent']['Version'])) {
                ...
                if (!isset($CIDRAM['Import']['CIDRAM Version'])) {
                    ...
                    if (isset($_POST['doConfig']) && $_POST['doConfig'] === 'on') {
                        if ($CIDRAM['Operation']->singleCompare($CIDRAM['Import']['CIDRAM Version'], '<1.23|>=2 <2.10|>=3')) {
                            ...
                            if ($CIDRAM['Operation']->singleCompare($CIDRAM['Import']['CIDRAM Version'], '^1.23')) {
                                /** Renamed configuration directives (v1->v2). */
                                foreach ([
                                    'general' => [
                                    ...
                                    foreach ($CIDRAM['Cat'] as $CIDRAM['Pair']) {
                                        if (isset($CIDRAM['Import']['Configuration'][$CIDRAM['CatKey']][$CIDRAM['Pair'][0]]) && !isset($CIDRAM['Import']['Configuration'][$CIDRAM['CatKey']][$CIDRAM['Pair'][1]])) {
                                            $CIDRAM['Import']['Configuration'][$CIDRAM['CatKey']][$CIDRAM['Pair'][1]] = $CIDRAM['Import']['Configuration'][$CIDRAM['CatKey']][$CIDRAM['Pair'][0]];
                                            unset($CIDRAM['Import']['Configuration'][$CIDRAM['CatKey']][$CIDRAM['Pair'][0]]);
                                        }
                                    }
                                }
                                unset($CIDRAM['Pair'], $CIDRAM['Cat'], $CIDRAM['Import']['Configuration']['general']['disable_cli']);
                            }
    ...
    if (!$CIDRAM['FE']['ASYNC']) {
        ...
        $CIDRAM['FE']['Data'] = '<div class="s">' . sprintf(
            ...
            '<span class="txtRd">' . $CIDRAM['NumberFormatter']->format($CIDRAM['FE']['SL_Signatures'] ?? 0) . '</span>',
            '<span class="txtRd">' . $CIDRAM['NumberFormatter']->format($CIDRAM['FE']['SL_Sections'] ?? 0) . '</span>',
            '<span class="txtRd">' . $CIDRAM['NumberFormatter']->format($CIDRAM['FE']['SL_Files'] ?? 0) . '</span>',
            '<span class="txtRd">' . $CIDRAM['NumberFormatter']->format($CIDRAM['FE']['SL_Unique'] ?? 0) . '</span>'
    ...
    if (isset($_POST['ip-addr'])) {
        ...
        $_POST['ip-addr'] = array_unique(array_map(function ($IP) use (&$CIDRAM) {
            $New = preg_replace(['~([.:])x(?:/.+)?$~i', '~[/, ].*$|&[a-z]+;|(?!.*:)(?<!:)(?<!:.)(?<!:..)(?<!:...)(?<!:....)[^\d.](?!.*:)|(?!.*\.)(?<!\.)(?<!\..)(?<!\...)(?<!\....)[^\da-f:](?!.*\.)|[. ]+$~i', '~\.{2,}~', '~:{3,}~'], ['{\1}0', '', '.', '::'], $IP);
        ...
        foreach ($_POST['ip-addr'] as $CIDRAM['ThisIP']['IPAddress']) {
            ...
            if (!empty($CIDRAM['Config']['recaptcha']['forcibly_disabled'])) {
                $CIDRAM['ThisIP']['YesNo'] .= '<br />++' . $CIDRAM['L10N']->getString('label_aux_special_recaptcha_disable');
            }
            if (!empty($CIDRAM['Config']['hcaptcha']['enabled'])) {
                $CIDRAM['ThisIP']['YesNo'] .= '<br />++' . $CIDRAM['L10N']->getString('label_aux_special_hcaptcha_mark');
            }
            if (!empty($CIDRAM['Config']['hcaptcha']['forcibly_disabled'])) {
                $CIDRAM['ThisIP']['YesNo'] .= '<br />++' . $CIDRAM['L10N']->getString('label_aux_special_hcaptcha_disable');
            }
            if (!empty($CIDRAM['Suppress output template'])) {
                $CIDRAM['ThisIP']['YesNo'] .= '<br />++' . $CIDRAM['L10N']->getString('label_aux_special_suppress');
            }
            if (!empty($CIDRAM['Suppress logging'])) {
                $CIDRAM['ThisIP']['YesNo'] .= '<br />++' . $CIDRAM['L10N']->getString('label_aux_special_suppress_logging');
            }
            if (isset($CIDRAM['Tracking options override'])) {
                if ($CIDRAM['Tracking options override'] === 'extended') {
                    ...
                    $CIDRAM['ThisIP']['YesNo'] .= '<br />++' . $CIDRAM['L10N']->getString('label_aux_special_tracking_default');
                }
            }
            ...
            $CIDRAM['FE']['IPTestResults'] .= $CIDRAM['ParseVars']($CIDRAM['L10N']->Data + $CIDRAM['ThisIP'], $CIDRAM['FE']['IPTestRow']);
        }
        unset($CIDRAM['ThisIP']);
    } else {
        $CIDRAM['FE']['ip-addr'] = '';
    ...
    $CIDRAM['FE']['LogFiles'] = ['Files' => $CIDRAM['arrayReplaceKeys']($CIDRAM['Logs-RecursiveList']($CIDRAM['Vault'], $CIDRAM['FE']['SortOrder']), function (array $Item): string {
    ...
    $CIDRAM['FE']['BlockLink'] = sprintf(
        ...
        isset($CIDRAM['QueryVars']['logfile'], $CIDRAM['FE']['LogFiles']['Files'][$CIDRAM['QueryVars']['logfile']]) ? '&logfile=' . $CIDRAM['QueryVars']['logfile'] : ''
    );
    ...
    } elseif (!isset($CIDRAM['QueryVars']['logfile'], $CIDRAM['FE']['LogFiles']['Files'][$CIDRAM['QueryVars']['logfile']])) {
    ...
    $CIDRAM['FE']['TextModeSwitchLink'] = sprintf(
        ...
        isset($CIDRAM['QueryVars']['logfile'], $CIDRAM['FE']['LogFiles']['Files'][$CIDRAM['QueryVars']['logfile']]) ? $CIDRAM['QueryVars']['logfile'] : '',
