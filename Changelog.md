...
*v4.0.0 major release tagged/released 2025.10.26.*
...
- The configuration directive for statistics has been split in two (tracking statistical information for CAPTCHAs now has its own configuration directive, separate from the other statistics), and the configuration for tracking statistical information for CAPTCHAs now distinguishes between platforms and can also track served CAPTCHAs (#624). After updating, both directives should be reviewed, as the values held previously by the former statistics directive won't be preserved.
...
- Configuration directive `ban_override` removed. Configuration directive `http_response_header_code` reworked as a matrix, now able to set different HTTP response codes depending on whether a request was blocked due to a ban, due to legal reasons, or due to other reasons (#624).
...
- More detailed information about errors generated by modules when using the IP testing page is provided now.
...
- [2026.04.20]: Added the ability to duplicate files at the front-end file manager.
...
- [2026.08.16~17]: At the IP testing page, when test results cite profiles, at the lines for any profiles generated by signature files, by modules or by auxiliary rules, the database SVG, the modules SVG, or the auxiliary rules SVG will now be displayed alongside those relevant profiles respectively to make it easier to distinguish those profiles from one another and from profiles generated elsewhere.
...
- [2026.09.10]: The AbuseIPDB module's reporterFinished event handler would sometimes interpret successful individual reports made to the AbuseIPDB API as having failed due to a wrong variable used when checking the response; Fixed (#691).
...
- [2026.09.05]: Reworked the permissions system so as to be somewhat more granular than before. Previously, an account could either have full access, access to only the logs page, or in the case of "Cronable" accounts, access to only the updates page. Accounts can now be defined as having complete access, access only to the logs page, to the statistics page, to the IP testing page, to the range tools (the calculator, the aggregator, the intersector, and the subtractor), to the glossary, or to a specified combination of one or more of those, or, as continues to be the case, for "Cronable" accounts, access to only the updates page. My recommendation to avoid publicly sharing front-end login endpoints and to avoid granting any more access than is absolutely necessary to anyone remains unchanged, but in theory, these changes should nonetheless be helpful, e.g., if, for some reason, somebody truly needed to share access to some of the range tools pages of their CIDRAM installation to a third-party because said third-party is absolutely unable to just install their own copy at their own server, however unlikely that may be, it's at least possible to do that now without also granting complete access to everything else.
