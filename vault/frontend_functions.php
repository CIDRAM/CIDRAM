...
$CIDRAM['SimulateBlockEvent'] = function ($Addr, $Modules = false, $Aux = false, $Verification = false) use (&$CIDRAM) {
    ...
    $CIDRAM['BlockInfo'] = [
        ...
        'Query' => isset($CIDRAM['FE']['custom-query']) ? str_replace(['&quot;', '&gt;', '&lt;', '&amp;'], ['"', '>', '<', '&'], $CIDRAM['FE']['custom-query']) : 'SimulateBlockEvent',
        'Referrer' => isset($CIDRAM['FE']['custom-referrer']) ? str_replace(['&quot;', '&gt;', '&lt;', '&amp;'], ['"', '>', '<', '&'], $CIDRAM['FE']['custom-referrer']) : 'SimulateBlockEvent',
 ...
 */
$CIDRAM['numberJs'] = function () use (&$CIDRAM) {
    return sprintf(
 ...
 * @return void
 */
$CIDRAM['UpdatesHandler-Update'] = function ($ID, $NotUsed = null) use (&$CIDRAM) {
    $CIDRAM['Arrayify']($ID);
    ...
    foreach ($ID as $ThisTarget) {
        ...
        if (
            ...
            for ($Iterate = 0; $Iterate < $Count; $Iterate++) {
                ...
                $CIDRAM['RemoteFiles'][$ThisFileName] = true;
                $ThisFile = '';
            }
            if ($Rollback) {
...
$CIDRAM['RangeTablesHandler'] = function (array $IPv4, array $IPv6) use (&$CIDRAM) {
    ...
    foreach ($SigTypes as $SigType) {
        ...
        $RangeCatOptions[] = '<option value="' . $Class . '"' . (isset($Styling[$SigType]) ? $Styling[$SigType] : '') . '>' . $SigType . '</option>';
...
$CIDRAM['AuxGenerateFEData'] = function ($Mode = false) use (&$CIDRAM) {
    ...
    foreach ($CIDRAM['AuxData'] as $Name => $Data) {
        ...
        if ($Mode) {
            ...
            $Output .= sprintf('<div class="iCntr"><div class="iLabl s">%1$s</div><div class="iCntn">', $CIDRAM['L10N']->getString('label_aux_http_status_code_override')) . sprintf(
            ...
            $Output .= sprintf(
                '<div class="iCntr"><div class="iLabl"><select name="mtd[%s]" class="auto" onchange="javascript:changeIfOrNotEditMode(this)"><option value="mtdStr"%s>%s</option><option value="mtdReg"%s>%s</option><option value="mtdWin"%s>%s</option><option value="mtdDMA"%s>%s</option></select><br /></div></div><div class="iCntr"><div class="iLabl"><em>%s</em></div></div>',
                $Current,
                $MethodData[0],
                $CIDRAM['FE']['optMtdStr'],
                $MethodData[1],
 ...
 */
$CIDRAM['ProcessRLUsage'] = function ($Data) {
    $Pos = 0;
    $EoS = strlen($Data);
    $Out = [];
    while ($Pos < $EoS) {
 ...
 * @return void
 */
$CIDRAM['ProcessMinifiedFormData'] = function ($MinifiedKey) use (&$CIDRAM) {
 ...
 */
$CIDRAM['arrayReplaceKeys'] = function (array $Arr, $Perform) {
    if (!is_callable($Perform)) {
        return [];
    }
    $Out = [];
    foreach ($Arr as $Item) {
 ...
 */
$CIDRAM['escapeJsInHTML'] = function ($In) {
    return str_replace(['"', '<', '>', '\\\\n'], ['&#34;', '&lt;', '&gt;', '\\n'], addslashes($In));
};
