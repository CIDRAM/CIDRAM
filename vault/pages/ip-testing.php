    ...
    foreach ($Working as $this->CIDRAM['ThisIP']['IPAddress']) {
            ...
            if (
                ...
                foreach ($this->CIDRAM['ThisIP']['Matches'][1] as $this->CIDRAM['ThisIP']['ThisMatch']) {
                    $this->CIDRAM['ThisIP']['YesNo'] = \str_replace(
                        ...
                        \sprintf(
                            '<ruby style="ruby-position:under"><a href="%1$s&search=%2$s" title="%3$s"><span class="flag %3$s"><span></span></span></a><rp>[</rp><rt class="flagruby" style="position:relative;bottom:-10px">%3$s</rt><rp>]</rp></a></ruby>',
                            $this->FE['CachedLogsLink'],
                            $this->preparePartForSearchLink($this->CIDRAM['ThisIP']['ThisMatch']),
                            $this->CIDRAM['ThisIP']['ThisMatch']
                        ),
        ...
        if (\is_array($this->Profiles) && \count($this->Profiles)) {
            foreach ($this->Profiles as $Profile) {
                ...
                if (isset($this->ProfileSources[$Profile])) {
                    $AuxClassColour = $this->CIDRAM['ThisIP']['StatClass'] === 'txtOe' ? 'auxbl' : 'aux' . \strtolower(\substr($this->CIDRAM['ThisIP']['StatClass'], -2));
                    if ($this->ProfileSources[$Profile] === 'Aux') {
                        $this->CIDRAM['ThisIP']['YesNo'] .= ' <span class="fmicon ' . $AuxClassColour . ' toolTip auxiliary" title="' . $this->L10N->getString('label.This profile was generated by an auxiliary rule') . '"></span>';
                    } elseif ($this->ProfileSources[$Profile] === 'Sig') {
                        $this->CIDRAM['ThisIP']['YesNo'] .= ' <span class="fmicon ' . $AuxClassColour . ' toolTip database" title="' . $this->L10N->getString('label.This profile was generated by a signature file') . '"></span>';
                    } elseif ($this->ProfileSources[$Profile] === 'Verification') {
                        $this->CIDRAM['ThisIP']['YesNo'] .= ' <span class="fmicon ' . $AuxClassColour . ' toolTip verification" title="' . $this->L10N->getString('label.This profile was generated due to verification') . '"></span>';
                    } elseif (($Try = $this->L10N->getString('Name.' . $this->ProfileSources[$Profile])) !== '') {
                        $this->CIDRAM['ThisIP']['YesNo'] .= ' <span class="fmicon ' . $AuxClassColour . ' toolTip module" title="' . \sprintf(
                            $this->L10N->getString('label.Generated by %s'),
                            \sprintf($this->L10N->getString('label.Quote entity'), $this->ProfileSources[$Profile])
                        ) . '"></span>';
                    }
                }
            }
