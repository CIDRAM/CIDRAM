...
1) 里面的`config.ini`文件，找到指令`disable_frontend`，并将其设置为`true` （默认值为`false`）。
...
/vault/cidramblocklists.dat | 包含的相关信息关于可选的国家封锁清单由Macmathan提供；由更新功能使用由前端提供。
...
“log_banned_ips”
- 包括IP禁止从阻止请求在日志文件吗？ True = 是【标准】； False = 不是。

“default_dns”
- 以逗号分隔的DNS服务器列表，用于主机名查找。 标准 = “8.8.8.8,8.8.4.4” (Google DNS)。 警告： 不要修改此除非您知道什么您做着！

“search_engine_verification”
- 尝试验证来自搜索引擎的请求？ 验证搜索引擎确保他们不会因超过违规限制而被禁止 （禁止在您的网站上使用搜索引擎通常会有产生负面影响对您的搜索引擎排名，SEO，等等）。 当被验证，搜索引擎可以被阻止，但不会被禁止。 当不被验证，他们可以由于超过违规限制而被禁止。 另外，搜索引擎验证提供保护针对假搜索引擎请求和针对潜在的恶意实体伪装成搜索引擎（当搜索引擎验证是启用，这些请求将被阻止）。 True = 搜索引擎验证是启用【标准】； False = 搜索引擎验证是禁用。

“protect_frontend”
- 指定是否应将CIDRAM通常提供的保护应用于前端。 True = 是【标准】； False = 不是。
...
“track_mode”
- 什么时候应该对违规行为进行计数？ False = 当IP被模块阻止时。 True = 当IP由于任何原因阻止时。
