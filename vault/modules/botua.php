...
$this->CIDRAM['ModuleResCache'][$Module] = function () {
    ...
    if ($this->trigger(\strlen($this->BlockInfo['UA']) > 4096, 'Bad UA', 'User agent string is too long', $UnmarkCaptcha)) {
    ...
    $this->trigger(\preg_match(
        ...
        'p(?:age(?:fetch|gett|_verifi)er|agesinventory|ath2|ic(?:grabber|s|tsnapshot|turefinder)|i(?:xmatch|xray)|oe-component-client-|owermarks|rofiler|(?:s|ure)bot|urity)|qqdownload|' .
        ...
        'yoofind~',
        $UANoSpace
    ), 'Backlink/SEO/Scraper UA'); // 2022.09.19 mod 2026.09.22
    ...
    if ($this->trigger(\preg_match(
        ...
        'c(?:arynai|asperbot|cbot|harstar|hatglm|hinaclaw|lark-?crawler|laude|ognitive|ohere-|ommoncrawl|ontent(?:atscale|bot|edge|harmony|king|samurai|shake)|onversionai|opilot|opy(?:ai|matic|scape)|oreweave|orrectiverag|otoyogi|rawl[4q]ai|rawler4j|rewai|rushonai)|' .
        ...
        'x(?:ai|tractorpro)|' .
        'yak/|' .
        'z(?:ephuli-?bot|grab|hipuai|huqueai)~',
        $UANoSpace
    ...
    if ($this->trigger(\preg_match('~crusader[-_]?worker~', $UANoSpace), 'Brute-force hacktool', '', $UnmarkCaptcha)) {
        $this->Reporter->report([15, 18, 19, 21], ['Brute-force hacktool detected.'], $this->BlockInfo['IPAddr']);
        $this->CIDRAM['Tracking options override'] = 'extended';
    } // 2026.08.05

    if (
        (\strpos($this->BlockInfo['WhyReason'], 'CVE-2026-4020') === false && $this->trigger(\preg_match('~vuln[-_]?scanner/|2026-4020~i', $UANoSpace), $Exploit = 'CVE-2026-4020', '', $UnmarkCaptcha)) || // 2026.08.05 mod 2026.08.06
        $this->trigger(\preg_match('~wp2shell[-_]?check/~i', $UANoSpace), $Exploit = 'CVE-2026-60137/CVE-2026-63030', '', $UnmarkCaptcha) // 2026.08.05
    ) {
