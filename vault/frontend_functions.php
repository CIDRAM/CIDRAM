 ...
 */
$CIDRAM['Formatter'] = function (string &$In, string $BlockLink = '', string $Current = '', string $FieldSeparator = ': ', bool $Flags = false) use (&$CIDRAM) {
...
$CIDRAM['SectionsHandler'] = function (array $Files) use (&$CIDRAM): string {
    ...
    foreach ($SectionMeta as $Section => $Counts) {
        $ThisCount = $CIDRAM['NumberFormatter']->format($SignaturesCount[$Section] ?? 0);
