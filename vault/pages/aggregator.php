 ...
 * @see LICENSE.txt
 *
 * This file: The aggregator page (last modified: 2026.09.05).
 */

namespace CIDRAM\CIDRAM;

if (!isset($this->CIDRAM['QueryVars']['cidram-page']) || $this->CIDRAM['QueryVars']['cidram-page'] !== 'aggregator' || empty($this->FE['PermissionsMap']['Range tools'])) {
    die;
}
...
if (!empty($_POST['input'])) {
    $this->FE['input'] = \str_replace("\r", '', \trim($_POST['input']));
    ...
    if ($Preserve) {
        ...
        $this->FE['output'] = \trim($StrObject->recompile());
        unset($StrObject);
    } else {
