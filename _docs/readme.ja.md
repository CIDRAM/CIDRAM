...
- ２.[インストール方法](#SECTION２）
- ３.[使用方法](#SECTION３）
- ４.[フロントエンドの管理](#SECTION４）
- ５.[本パッケージに含まれるファイル](#SECTION５）
- ６.[設定オプション](#SECTION６）
- ７.[シグネチャ（署名）フォーマット](#SECTION７）
- ８.[よくある質問（ＦＡＱ）](#SECTION８）
...
- **[Spambot Securityが提供するCIDRAMサポート・フォーラム](http://www.spambotsecurity.com/forum/viewforum.php?f=6１）** をチェックしましたか？​問題が以前に言及されているかどうかをチェックしてください。​提案、​アイデア、​ソリューションが提供されたかどうかをチェックしてください。
