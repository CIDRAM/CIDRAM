...
Эта информация относится к самой последней версии v1, и может не иметь отношения к другим версиям или выпускам.
...
- Не забывайте, что CIDRAM записывает кеш и другие данные непосредственно на диск (т.е., сохраняет свои данные в файлах), и не использует внешнюю систему баз данных, такую как MySQL, PostgreSQL, Access, или подобное. Это означает, что для того, чтобы отслеживать использование для ограничения скорости, было бы действительно нужно записывать на диск для каждого отдельного запроса с ограничением скорости. Это может способствовать снижению продолжительности жизни диска в долгосрочной перспективе и в идеале не рекомендуется. Вместо этого, в идеале, инструмент, используемый для ограничения скорости, может использовать систему баз данных, предназначенную для частых, небольших операций чтения/записи, или может поддерживать информацию во всех запросах, без необходимости записи данных на диск между запросами (например, написанный как независимый серверный модуль, вместо пакета PHP).
...
Последнее обновление: 16 Апрель 2019 (2019.04.16).
