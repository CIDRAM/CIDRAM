...
class Operation
{
     ...
     */
    public function opVersions($Actual, $Constraint, $NextMajor, $GreaterThan, $OrEqualTo)
    {
     ...
     * @return bool True if constraint is met; False otherwise.
     */
    public function opEqualTo($Actual, $Constraint)
    {
        $Actual = $this->splitVersionParts($Actual);
        $Constraint = $this->splitVersionParts($Constraint);
        while (true) {
     ...
     */
    public function multiCompare(array $Operand, array $Prefix)
    {
        $Result = false;
     ...
     * @return bool Return value of the operation.
     */
    public function singleCompare($Operand, $Prefix)
    {
     ...
     */
    public function splitVersionParts($Version = '')
    {
     ...
     */
    public function ifCompare(&$Data, $IfString)
    {
        $LCIfString = \strtolower($IfString);

        /** Guard. */
        if (\substr($LCIfString, 0, 3) !== 'if ') {
            $IfString = \trim($IfString);
