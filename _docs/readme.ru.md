...
/vault/ignore.dat | Игнорируется файл (используется для указания подписи секции CIDRAM следует игнорировать).
...
"block_generic"
- Блокировать CIDRs рекомендуется для черных списков? Это охватывает любые подписи, которые не помечены как часть какой-либо из других более конкретных категорий подписи.
...
Заметка: A reCAPTCHA instance will ONLY be offered to the user if reCAPTCHA is enabled (either with "usemode" as 1, or "usemode" as 2 with "enabled" as true), and if exactly ONE signature has been triggered (no more, no less; if multiple signatures are triggered, a reCAPTCHA instance will NOT be offered).
...
В контексте CIDRAM, эти термины относятся к подписи CIDRAM и что/кого они блокируют. Когда CIDRAM блокирует IP-адрес из-за плохой, устаревшей или неправильной подписи, но не должно быть сделано, или когда он делает это по неправильным причинам, мы называем это событие как "ложноположительный". Когда CIDRAM не удается блокировать IP-адрес, который должен был быть заблокированы, из-за непредвиденных угроз, недостающие подписи или недостатки в своих подписей, мы называем это событие как "обнаружение не удалось" (аналогична "ложноотрицательный").
...
Последнее обновление: 28 Сентябрь 2016 (2016.09.28).
